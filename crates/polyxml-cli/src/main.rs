...
fn run_generate(args: GenerateArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for lang in &languages {
        ...
        for (schema_path, ir) in &compiled_schemas {
            emit_target_placeholder(lang, &lang_out, schema_path, ir)?;
        }
...
fn run_build(args: BuildArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for target in &targets {
        ...
        for (schema_path, ir) in &compiled_schemas {
            emit_target_placeholder(&target.target, &target_dir, schema_path, ir)?;
        }
...
fn run_language_formatter(lang: &str, dir: &Path) {
    let cmd = match lang.to_lowercase().as_str() {
        "python" | "py" => ("ruff", vec!["format", dir.to_str().unwrap()]),
        "rust" | "rs" => ("cargo", vec!["fmt"]),
        "go" => ("gofmt", vec!["-w", dir.to_str().unwrap()]),
        "cpp" | "c++" => ("clang-format", vec!["-i"]),
        _ => return,
    };

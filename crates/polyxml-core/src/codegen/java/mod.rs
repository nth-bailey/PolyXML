...
/// Code generator producing Java 21+ records, sealed interfaces, and enums from SchemaIR.
...
impl JavaCodegen {
    ...
    pub fn generate_files(&self, ir: &SchemaIR) -> Vec<(String, String)> {
        ...
        for type_def in ir.types.values() {
            let (type_name, content) = self.generate_single_type(type_def, false);
    ...
    pub fn generate_module(&self, ir: &SchemaIR, outer_class_name: &str) -> String {
        ...
        for type_def in ir.types.values() {
            let (_, body) = self.generate_type_body(type_def, "    ");
            out.push_str(&body);
            out.push('\n');
        }
    ...
    fn generate_single_type(&self, type_def: &TypeDef, is_nested: bool) -> (String, String) {
        let mut out = String::new();
        if !is_nested {
            self.emit_file_header(&mut out);
        }

        let (type_name, body) = self.generate_type_body(type_def, "");
        out.push_str(&body);
        (type_name, out)
    }

    fn generate_type_body(&self, type_def: &TypeDef, indent: &str) -> (String, String) {
        let mut out = String::new();
        let type_name = match type_def {
            TypeDef::Struct(s) => {
                let name = to_java_type_name(&s.qname.local);
                self.emit_struct(&mut out, s, &name, indent);
                name
            }
            TypeDef::Enum(e) => {
    ...
    fn emit_struct(&self, out: &mut String, s: &StructDef, java_name: &str, indent: &str) {
        ...
        for field in &s.fields {
            ...
            let field_type = if is_list {
                format!(
                    "java.util.List<{}>",
                    self.context.boxed_type(&field.type_ref)
                )
            } else if is_optional {
                format!(
                    "java.util.Optional<{}>",
                ...
                self.context.map_type_ref(&field.type_ref)
            };
    ...
    fn emit_simple(&self, out: &mut String, s: &SimpleTypeDef, java_name: &str, indent: &str) {
        ...
        if checks.is_empty() {
            let _ = writeln!(
                out,
                "{}{}record {}({} value) {{}}",
                indent, modifier, java_name, base_type
            );
            return;
        }
